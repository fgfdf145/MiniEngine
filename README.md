...
- glTF：第二套 UV、sampler 与 `KHR_texture_transform` 已导入（见下方对应条目），但只读两套 UV，要求第三套及以上的贴图改用第二套并告警。动画、蒙皮与 morph target 不导入（2026-09-26 定为不在当前范围）。x86 Windows 构建没有 libktx，读不了 KTX2 纹理。
...
- 渲染端已有 `alphaMode` 分类（opaque / mask / blend × 单双面共 6 条管线变体）与半透明 back-to-front 排序；抗锯齿有 TAA 与 specular AA，环境光有漫反射与镜面 IBL（Atmosphere / HDRI 模式，见下方条目）。仍没有视锥剔除；局部光阴影受图集容量限制（最多 64 块，点光和面光各占 6 块），面光按中心点投影、半影不随面积变宽；显存按每 submesh 独立分配（`vkAllocateMemory`，没有子分配器）。缺口清单见 2026-07-30 的开发记录，其中管线相关两条已在 2026-09-03 处理，「缺失特性」中无独立 HDR 中间靶、色调映射硬编码在 `triangle.frag` 一条已在 2026-09-12 处理。不透明与 Mask 几何走 G-Buffer 延迟着色，前向着色材质、透射与 Blend 走前向；GB4 实体 id 拾取（第三阶段）尚未实现。
- G-Buffer 带有相机与物体运动的 motion vector（`.rg`，当前 UV 减上一帧 UV；目标自 BRDF 第 2 阶段起为 `R16G16B16A16_SFLOAT`，`.ba` 存清漆法线；上一帧 model 矩阵经 set 0 binding 2 的 SSBO 按 `firstInstance` 索引），可在 Graphics Debug 窗口查看；Blend 表面与背景像素没有速度。TAA 与 AO 的时域累积用它重投影历史。设计见 [docs/superpowers/specs/2026-09-19-motion-vectors-design.md](docs/superpowers/specs/2026-09-19-motion-vectors-design.md)。
...
- 模型自带灯光（`KHR_lights_punctual`）：导入每盏平行光、点光、聚光及其节点的世界变换（位置、沿节点 -Z 的方向）；单位换成引擎的：点光 `4π cd`、聚光 `2π(1 - cos 外角) cd` 换成流明，平行光的 lux 不变；未定义的范围（glTF 里是无穷）取光照降到 1e-3 lux 处，`sqrt(cd / 1e-3)`，上限 1000 m；默认聚光角（内 0、外 π/4）照 glTF。灯光属于模型而不是场景实体，经模型实体的变换加入场景灯光，所以存盘再读不会重复；实体上的 `ModelComponent::useModelLights`（默认开，场景里关掉时存为 `use_model_lights: false`）控制是否发光，场景面板对带灯的模型显示 Model Lights 勾选框。设计见 [docs/superpowers/specs/2026-09-26-gltf-loading-completeness-design.md](docs/superpowers/specs/2026-09-26-gltf-loading-completeness-design.md)。
- 压缩几何与 KTX2 纹理：Meshopt（`EXT_meshopt_compression` 与 `KHR_meshopt_compression`，vcpkg 的 meshoptimizer）在读任何 accessor 之前把每个压缩的 bufferView 解码（attributes / triangles / indices 三种模式，再做 octahedral、quaternion、exponential 或 colour 滤波）到新 buffer，文件里没有数据的 fallback buffer 在 tinygltf 解析前先补一个占位字节，从不读取；Draco（`KHR_draco_mesh_compression`，vcpkg 的 draco）把带扩展的图元解码，属性按 accessor 自己的分量类型写出（浮点属性由 draco 反量化，整数属性交给 accessor 的 normalized），索引写成 32 位；KTX2（`KHR_texture_basisu`，vcpkg 的 ktx）纹理优先取扩展里的 KTX2 图像，Basis Universal（ETC1S 或 UASTC）转码成 RGBA8，未压缩的 8 位 RGBA KTX2 直接读，之后照常重建 mip、压成 BC7/BC5 并缓存；.glb 内嵌的 KTX2 图像导入时解成 PNG。vcpkg 的 ktx 不支持 x86 Windows，那个构建里读 KTX2 纹理会报错说明。设计见 [docs/superpowers/specs/2026-09-26-gltf-loading-completeness-design.md](docs/superpowers/specs/2026-09-26-gltf-loading-completeness-design.md)。
- GPU 实例（`EXT_mesh_gpu_instancing`）：导入时把每个实例展开成网格的一份副本，按 `节点世界变换 × T × R × S` 烘进顶点（与节点变换的做法相同），名字后缀 `/instance_<i>`，不再画未实例化的原网格；ROTATION 可以是归一化的 byte/short。属性数量不一致时导入失败。这不是硬件实例化（渲染器已把 `gl_InstanceIndex` 用作 draw 槽位），内存随实例数增长，导入时记录实例数。Khronos 对比取景仍按节点变换下的网格包围盒，与 viewer 一致。同时修正：没有材质的图元改用 glTF 的默认材质（白色、金属度 1、粗糙度 1），作为模型材质之后单独的一项；此前它用的是金属度 0 的引擎默认值，而且模型有材质时会错拿第 0 个。SimpleInstancing 与 viewer 的平均差 2.1。设计见 [docs/superpowers/specs/2026-09-26-volume-scatter-gpu-instancing-design.md](docs/superpowers/specs/2026-09-26-volume-scatter-gpu-instancing-design.md)。
- Khronos 参考对比：Graphics Debug 的 Khronos reference view（命令行 `--khronos-reference`）按 Khronos glTF Sample Viewer 的默认设置渲染——Khronos PBR Neutral 色调映射、HDRI 纹素 1 曝光为 1、输出按 viewer 的 2.2 gamma 编码（而非 sRGB 曲线，暗部差别最大）、关闭自动白平衡、辉光、AO、SSR、specular AA 与阴影，模型自带的灯光（`KHR_lights_punctual`）由引擎随模型导入、背景用粗糙度 0.6 的预滤波环境图，并按 viewer 的规则取景（每个图元的 accessor 包围盒经节点变换后扩成外接立方体再求并）。`tools/khronos_reference/` 下载 glTF-Sample-Assets 测试模型、生成场景、批量截图，并把 viewer 截图与引擎截图并排、出差异图和平均差（用法见其 README）。2026-09-26 的首轮结果（26 个场景，平均差 0–255）：底色、金属度、粗糙度、法线、clearcoat、sheen、specular、iridescence、各向异性强度/旋转、emissive strength 与鞋的四个变体均在 2.7–4.6，即噪声底（TextureTransformTest 在补上 glTF sampler 后由 5.6 降到 2.7；CompareAnisotropy 由 6.1 降到 3.8、CompareNormal 由 3.4 降到 2.6，见下条切线修正）；已知差异为 CompareIor / IORTestGrid（12–15，需透射，第 4 阶段）、UnlitTest（5.8，viewer 不对 unlit 做色调映射，引擎有意经过色调曲线）。设计见 [docs/superpowers/specs/2026-09-26-khronos-reference-comparison-design.md](docs/superpowers/specs/2026-09-26-khronos-reference-comparison-design.md)。
...
- 缺少 NORMAL 时生成平面法线（2026-09-26）：glTF 规定没有 NORMAL 的图元必须用平面法线，此前导入时把相邻面法线平均成平滑法线，立方体的棱角因此被抹圆。现在每个三角形使用自己的三个顶点副本（连同 UV 等全部属性），法线取该三角形的面法线，顶点数最多变为原来的 3 倍；自带 NORMAL 的网格不变。测试见 `tests/tangent_generation_tests.cpp`。
- 透射与体积（`KHR_materials_transmission`、`KHR_materials_volume`，完整 BRDF 计划第 4a 阶段）：透射材质带标志位 128，不进 G-buffer，由拆开的第二个 forward pass（先透射物体、后 Blend，都从后往前）绘制；此前 `VulkanTransmissionCopyPass` 把已画好不透明物体和天空的 HDR 拷进固定 1024×1024、带完整 mip 链的图像（与 Khronos Sample Viewer 相同，LOD = log2(1024) × 材质粗糙度 × clamp(2·IOR − 2)）。着色时沿按 IOR 折射、走过厚度（乘节点与实体缩放）的光线取出射点投影采样，乘 Beer-Lambert 衰减和底色，按透射系数替换电介质漫反射；点、聚光、方向光另有绕表面镜像的 GGX 透射项。透射物体不投影、无 SSR/AO、不写运动向量。材质面板、sidecar（`transmission_factor`、`thickness_factor`、`attenuation_distance`、`attenuation_color` 及两张贴图）照其他扩展。设计与实现修订见 [docs/superpowers/specs/2026-09-26-transmission-volume-design.md](docs/superpowers/specs/2026-09-26-transmission-volume-design.md)。命令行 `--viewport-size 宽x高` 固定场景渲染尺寸，截图不再受编辑器布局影响。
- 色散与漫透射（`KHR_materials_dispersion`、`KHR_materials_diffuse_transmission`，第 4b/4c 阶段）：色散存在 `transmissionFactors.w`，有体积时 R/G/B 各按自己的 IOR（绿色两侧各展开 (IOR − 1) × 0.025 × dispersion，与 Sample Viewer 相同）折射、采样、模糊和衰减，玻璃与宝石出现彩边。漫透射（Release Candidate，viewer 已支持）存在材质第 14 个 vec4（颜色与系数）和两张新贴图（槽 25 系数 A、槽 26 颜色 RGB sRGB），材质走 forward 着色：环境光的漫反射按系数混向背面（-N）的辐照度乘透射颜色（同样乘材质 AO，与 viewer 一致）；每个从背面照来的光源加一个穿过表面的 Lambert 项，正面漫反射保留 1 − 系数；有体积时颜色按厚度 × 节点平均缩放衰减。会透光的表面被背光照射时，阴影查找的偏移改朝光源一侧，避免被自己挡住。材质面板、sidecar（`dispersion`、`diffuse_transmission_factor`、`diffuse_transmission_color` 及两张贴图）照其他扩展。设计与实现修订见 [docs/superpowers/specs/2026-09-26-dispersion-diffuse-transmission-design.md](docs/superpowers/specs/2026-09-26-dispersion-diffuse-transmission-design.md)。
- 体积散射（`KHR_materials_volume_scatter`，草案；完整 BRDF 计划第 5 阶段 SSS 中有 Khronos 标准的那一半）：按 Khronos Sample Viewer 的做法实现为屏幕空间 Burley 扩散（Blender 的实现）。导入 `multiscatterColor` 与 `scatterAnisotropy`（只在同时有 `KHR_materials_volume` 时生效；唯一的示例模型 ScatteringSkull 把颜色写成 `multiscatterColorFactor`，即后继提案 `KHR_materials_scatter` 的名字，缺少草案名时读它）；有漫透射（系数大于 0）才会散射，数据存在材质第 15 个 vec4，并以 `volumeScale.w` 标记。新的 `VulkanScatterPass` 在 forward pass 之前，用 forward 管线的着色器（特化常量 `kScatterPrepass`）只画散射材质，写进它自己的 RGBA16F 颜色和 D32 深度（随场景尺寸重建，经 set 0 的 19、20 号绑定采样）：颜色是进入表面的漫射光（正面按单次散射反照率，背面穿过体积、未被吸收的部分再散射，乘漫透射系数、菲涅耳与 sheen 缩放；与 viewer 不同，带阴影），alpha 是 draw 槽位。forward pass 把漫透射的背面光乘以（1 - 单次散射反照率），再加上扩散项：以“衰减距离 × multiscatterColor”的最大通道为半径，在屏幕上取 55 个 Burley 样本（黄金角、逆 CDF 牛顿迭代，与 viewer 的 `computeScatterSamples` 数值一致，由 C++ 测试核对），只取同一 draw 的样本，按两点间的 Burley 剖面除以 pdf 加权平均，再乘透射颜色和 viewer 的权重。与 viewer 有一处有意的差别：采样圆在像素上是圆的（viewer 两个轴都按宽度的纹素缩放）。`scatterAnisotropy` 只保存、不参与着色（viewer 也不用）。材质面板、sidecar（`volume_scatter`、`multiscatter_color`、`scatter_anisotropy`）照其他扩展。对比：viewer 只认草案名，原版 ScatteringSkull 在 viewer 里不散射（漆黑），所以另外对比一份把键名改成草案名的副本 ScatteringSkullDraftKey，平均差 2.4；原版的 28.9 属预期差异。设计见 [docs/superpowers/specs/2026-09-26-volume-scatter-gpu-instancing-design.md](docs/superpowers/specs/2026-09-26-volume-scatter-gpu-instancing-design.md)。
