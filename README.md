...
- 渲染：Cook-Torrance PBR（含多次散射能量补偿）、材质贴图、场景视口、多类型灯光（Directional、Point、Spot、Area、Ambient，最多 1024 盏，局部灯按分簇查找）及灯光 gizmo；最亮的方向光投射 4 级级联阴影（CSM，每级 2048²，3×3 双线性 PCF）。
...
- Bloom（Jimenez 2014，Call of Duty: Advanced Warfare 的做法；Graphics Debug 的 “Bloom” 与强度，默认开、0.04）：在 TAA 之后、两种顺序里都有的计算 pass `Bloom`，从半分辨率起建最多 6 级 mip（`BuildBloomMipChain`，`miniengine.bloom_chain` 覆盖），第一级 13 点降采样用 Karis 平均防止太阳圆盘这类单点闪烁，再用 3×3 tent 逐级升采样累加，最后以 `mix(场景, bloom / 级数, 强度)` 原地写回 `SceneTaa`：不设阈值，每个像素把 4% 的能量分给周围，画面平均亮度只变约 0.3%。mip 链由 pass 自己持有（一张 RGBA16F 图、每级一个 view、`GENERAL`），每帧丢弃重写，派发之间用内存屏障排序。关掉时截图与改动前的差异在噪声之内。已知限制：HDR 目标以 fp16 存未乘曝光的物理亮度，超过 65504 cd/m² 就被截断，日光曝光（EV 15 左右）下这只比白点高约 1.1 倍，所以白天场景里再亮的光源 bloom 也有限；Sponza 这种 EV 5 左右的场景余量约 1700 倍，效果正常。根治需要预曝光的 HDR 缓冲。设计见 [docs/superpowers/specs/2026-09-24-bloom-design.md](docs/superpowers/specs/2026-09-24-bloom-design.md)。
