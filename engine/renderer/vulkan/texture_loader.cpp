...
TextureData LoadPortablePixmap(const std::string& path)
{
    ...
    {
        const int r = std::stoi(ReadNextPortableMapToken(input));
        const int g = std::stoi(ReadNextPortableMapToken(input));
        const int b = std::stoi(ReadNextPortableMapToken(input));

        texture.pixels[static_cast<size_t>(i) * 4 + 0] = static_cast<std::uint8_t>(r * scale);
        texture.pixels[static_cast<size_t>(i) * 4 + 1] = static_cast<std::uint8_t>(g * scale);
        texture.pixels[static_cast<size_t>(i) * 4 + 2] = static_cast<std::uint8_t>(b * scale);
