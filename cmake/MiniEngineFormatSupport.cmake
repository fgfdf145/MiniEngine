...
function(_miniengine_decode_bash_ansi_c_escape escape_text
    ...
    elseif(escape_code STREQUAL "u" OR escape_code STREQUAL "U")
        if(escape_code STREQUAL "u")
            set(maximum_digits 4)
        else()
            set(maximum_digits 8)
        endif()
        set(hex_digits "")
        set(position 1)
        while(position LESS escape_length AND
              position LESS_EQUAL maximum_digits)
            string(SUBSTRING "${escape_text}" ${position} 1 digit)
            if(digit MATCHES "^[0-9A-Fa-f]$")
                string(APPEND hex_digits "${digit}")
                math(EXPR position "${position} + 1")
    ...
    elseif(escape_code STREQUAL "c" AND escape_length GREATER_EQUAL 2)
        ...
        string(HEX "${control_character}" control_hex)
        string(LENGTH "${control_hex}" control_hex_length)
        if(control_character STREQUAL "?")
            set(control_value 127)
        elseif(control_hex_length EQUAL 2)
            math(EXPR control_value "0x${control_hex} & 31")
        else()
            set(control_value -1)
        endif()
