...
// 8 x vec4). triangle.frag and gbuffer.frag index it with the draw slot triangle.vert forwards.
struct MaterialData
{
    ...
    uvec4 shadingModel;      // x = SHADING_MODEL_* layer | SHADING_MODEL_ANISOTROPY_BIT (gbuffer_common.glsl); yzw reserved
    vec4 clearcoatFactors;   // x clearcoat factor, y clearcoat roughness; read for SHADING_MODEL_CLEARCOAT
    vec4 sheenFactors;       // rgb sheen colour, a sheen roughness; read for SHADING_MODEL_SHEEN
    vec4 anisotropyFactors;  // x strength, y cos rotation, z sin rotation; read with SHADING_MODEL_ANISOTROPY_BIT
};
